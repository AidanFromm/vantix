...
function Navigation() {
  ...
  useEffect(() => {
    const onScroll = () => {
      // Transition nav after scrolling past ~80% of viewport (hero area)
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? 'bg-[#F4EFE8]/95 backdrop-blur-sm border-b border-[#E3D9CD]'
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 py-4 flex items-center justify-between">
        <a href="/" className="flex items-center gap-2.5">
          ...
          <span className={`text-2xl font-extrabold tracking-tight transition-colors duration-500 ${
            scrolled ? 'text-[#B07A45]' : 'text-white'
          }`}>vantix<span className={scrolled ? 'text-[#8E5E34]' : 'text-[#B07A45]'}>.</span></span>
        </a>
        <div className="hidden md:flex items-center gap-8">
          {links.map((l) => (
            <a key={l.href} href={l.href} className={`text-sm transition-colors duration-500 ${
              scrolled ? 'text-[#7A746C] hover:text-[#B07A45]' : 'text-white/60 hover:text-white'
            }`}>
              {l.label}
            </a>
          ))}
        ...
        <button className={`md:hidden transition-colors duration-500 ${scrolled ? 'text-[#B07A45]' : 'text-white'}`} onClick={() => setMobileOpen(!mobileOpen)} aria-label="Toggle menu">
