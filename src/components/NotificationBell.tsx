  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="relative p-2 rounded-lg text-[#7A746C] hover:text-[#1E1E1E] hover:bg-[#EFE6DA] transition-colors"
        ...
        <AnimatePresence>
          {unread > 0 && (
            <motion.span
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0 }}
              className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-[#9B6C3C] text-white text-[10px] font-bold px-1"
            >
              {unread > 99 ? '99+' : unread}
            </motion.span>
          )}
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#D8CFC4]">
              ...
              <div className="flex items-center gap-1">
                {unread > 0 && (
                  <button onClick={handleMarkAllRead} className="p-1.5 rounded-md text-[#7A746C] hover:text-[#1E1E1E] hover:bg-[#EFE6DA] transition-colors" title="Mark all read">
                    <CheckCheck size={16} />
                  </button>
                )}
                ...
                <button onClick={() => setOpen(false)} className="p-1.5 rounded-md text-[#7A746C] hover:text-[#1E1E1E] hover:bg-[#EFE6DA] transition-colors">
                  <X size={16} />
                </button>
              </div>
            </div>
                ...
                recent.map((n) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left flex gap-3 px-4 py-3 hover:bg-[#EFE6DA] transition-colors border-b border-[#D8CFC4] last:border-b-0 ${!n.read ? 'bg-[#9B6C3C]/[0.04]' : ''}`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className={`text-sm font-medium truncate ${!n.read ? 'text-[#1E1E1E]' : 'text-[#7A746C]'}`}>{n.title}</span>
                          {!n.read && <span className="w-2 h-2 rounded-full bg-[#9B6C3C] flex-shrink-0" />}
                        </div>
            ...
            {notifications.length > 0 && (
              <Link
                ...
                className="block text-center py-2.5 text-xs text-[#9B6C3C] hover:bg-[#EFE6DA] transition-colors border-t border-[#D8CFC4] font-medium"
              >
