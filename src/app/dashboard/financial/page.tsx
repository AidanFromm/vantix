...
export default function FinancialPage() {
  ...
  if (loading) return <div className="flex items-center justify-center h-64"><div className="w-8 h-8 border-2 border-[#B8895A] border-t-transparent rounded-full animate-spin" /></div>;

  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-2xl sm:text-3xl font-bold text-[#2D2A26]">Financial Overview</h1><p className="text-[#8C857C] mt-1 text-sm">Revenue, expenses, and invoice management</p></div>
        <button onClick={downloadReport} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-[#8C857C] border border-[#E8E2DA] hover:text-[#2D2A26] hover:bg-[#F5F0EB] transition-colors text-sm font-medium"><Download size={16} /> Report</button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className={`bg-white border ${stat.border} rounded-2xl p-5 shadow-[4px_4px_12px_#d1cdc7,-4px_-4px_12px_#ffffff]`}>
            <div className="flex items-center justify-between mb-3"><span className="text-sm text-[#8C857C]">{stat.label}</span><div className={`w-10 h-10 rounded-xl ${stat.bg} flex items-center justify-center`}><stat.icon size={20} className={stat.color} /></div></div>
      ...
      {/* Invoices Table */}
      <div className="bg-white border border-[#E8E2DA] rounded-2xl overflow-hidden shadow-[4px_4px_12px_#d1cdc7,-4px_-4px_12px_#ffffff]">
        <div className="p-5 border-b border-[#E8E2DA]"><h2 className="text-lg font-semibold text-[#2D2A26] flex items-center gap-2"><FileText size={20} className="text-[#B8895A]" /> Invoices</h2></div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-[#E8E2DA] text-left text-[#8C857C] text-xs uppercase"><th className="px-5 py-4 font-medium">Client</th><th className="px-5 py-4 font-medium">Amount</th><th className="px-5 py-4 font-medium">Due Date</th><th className="px-5 py-4 font-medium">Status</th></tr></thead>
            <tbody>
              {invoices.length === 0 ? (
                <tr><td colSpan={4} className="text-center py-12 text-[#8C857C]">No invoices yet</td></tr>
              ) : invoices.map(inv => {
                ...
                return (
                  <tr key={inv.id} className="border-b border-[#E8E2DA]/50 hover:bg-[#F5F0EB]/50 transition-colors">
                    <td className="px-5 py-4"><p className="font-medium text-[#2D2A26]">{inv.client?.name || 'N/A'}</p><p className="text-xs text-[#8C857C]">{inv.invoice_number || inv.id.slice(0, 8)}</p></td>
                    <td className="px-5 py-4 font-semibold text-[#B8895A]">{formatCurrency(amt)}</td>
                    <td className="px-5 py-4 text-[#8C857C]">{inv.due_date ? new Date(inv.due_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}</td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-[#E8E2DA] rounded-2xl overflow-hidden shadow-[4px_4px_12px_#d1cdc7,-4px_-4px_12px_#ffffff]">
          <div className="p-5 border-b border-[#E8E2DA] flex items-center justify-between"><h2 className="text-lg font-semibold text-[#2D2A26] flex items-center gap-2"><Receipt size={20} className="text-amber-500" /> Recent Expenses</h2><span className="text-sm text-[#8C857C]">{formatCurrency(totalMonthlyExpenses)}</span></div>
            ...
            {expenses.length === 0 ? (
              ...
              <div key={expense.id} className="px-5 py-4 hover:bg-[#F5F0EB]/50 transition-colors flex items-center justify-between group">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <button onClick={() => handleDeleteExpense(expense.id)} className="p-2 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-red-50 text-[#8C857C] hover:text-red-500 transition-all">
        ...
        <div className="bg-white border border-[#E8E2DA] rounded-2xl p-5 shadow-[4px_4px_12px_#d1cdc7,-4px_-4px_12px_#ffffff]">
          <h2 className="text-lg font-semibold text-[#2D2A26] flex items-center gap-2 mb-5"><PieChart size={20} className="text-amber-500" /> Expense Breakdown</h2>
          {expenseBreakdown.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8"><PieChart size={32} className="text-[#8C857C]/30 mb-4" /><p className="text-[#8C857C] text-sm">No expense data</p></div>
          ) : (
              ...
              {expenseBreakdown.map(cat => {
                ...
                return (
                  <div key={cat.name} className="flex items-center justify-between">
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: cat.color }} /><span className="text-sm text-[#8C857C]">{cat.name}</span></div>
                    <div className="flex items-center gap-3"><div className="w-20 h-1.5 bg-[#F5F0EB] rounded-full overflow-hidden"><div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: cat.color }} /></div><span className="text-sm font-medium text-[#2D2A26] w-16 text-right">{formatCurrency(cat.amount)}</span></div>
                  </div>
                );
              })}
            </div>
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white border border-[#E8E2DA] rounded-2xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="text-xs text-[#8C857C] mb-1.5 block">Date</label><input type="date" value={form.expense_date} onChange={e => setForm(f => ({ ...f, expense_date: e.target.value }))} className={inputCls} /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
