...
export default function FinancialPage() {
  ...
  if (loading) return <div className="flex items-center justify-center h-64"><div className="w-8 h-8 border-2 border-[#B07A45] border-t-transparent rounded-full animate-spin" /></div>;

  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-2xl sm:text-3xl font-bold text-[#1E1E1E]">Financial Overview</h1><p className="text-[#A89F94] mt-1 text-sm">Revenue, expenses, and invoice management</p></div>
        <button onClick={downloadReport} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-[#A89F94] border border-[#D8CFC4] hover:text-[#1E1E1E] hover:bg-[#EFE6DA] transition-colors text-sm font-medium"><Download size={16} /> Report</button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className={`bg-white border ${stat.border} rounded-2xl p-5 shadow-sm`}>
            <div className="flex items-center justify-between mb-3"><span className="text-sm text-[#A89F94]">{stat.label}</span><div className={`w-10 h-10 rounded-xl ${stat.bg} flex items-center justify-center`}><stat.icon size={20} className={stat.color} /></div></div>
      ...
      <div className="bg-white border border-[#D8CFC4] rounded-2xl overflow-hidden shadow-sm">
        <div className="p-5 border-b border-[#D8CFC4]"><h2 className="text-lg font-semibold text-[#1E1E1E] flex items-center gap-2"><FileText size={20} className="text-[#B07A45]" /> Invoices</h2></div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-[#D8CFC4] text-left text-[#A89F94] text-xs uppercase"><th className="px-5 py-4 font-medium">Client</th><th className="px-5 py-4 font-medium">Amount</th><th className="px-5 py-4 font-medium">Due Date</th><th className="px-5 py-4 font-medium">Status</th></tr></thead>
            <tbody>
              {invoices.length === 0 ? (
                <tr><td colSpan={4} className="text-center py-12 text-[#A89F94]">No invoices yet</td></tr>
              ) : invoices.map(inv => {
                ...
                return (
                  <tr key={inv.id} className="border-b border-[#D8CFC4]/50 hover:bg-[#EFE6DA]/50 transition-colors">
                    <td className="px-5 py-4"><p className="font-medium text-[#1E1E1E]">{inv.client?.name || 'N/A'}</p><p className="text-xs text-[#A89F94]">{inv.invoice_number || inv.id.slice(0, 8)}</p></td>
                    <td className="px-5 py-4 font-semibold text-[#B07A45]">{formatCurrency(amt)}</td>
                    <td className="px-5 py-4 text-[#A89F94]">{inv.due_date ? new Date(inv.due_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}</td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-[#D8CFC4] rounded-2xl overflow-hidden shadow-sm">
          <div className="p-5 border-b border-[#D8CFC4] flex items-center justify-between"><h2 className="text-lg font-semibold text-[#1E1E1E] flex items-center gap-2"><Receipt size={20} className="text-amber-500" /> Recent Expenses</h2><span className="text-sm text-[#A89F94]">{formatCurrency(totalMonthlyExpenses)}</span></div>
          <div className="divide-y divide-[#D8CFC4] max-h-80 overflow-y-auto">
            {expenses.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12"><Receipt size={24} className="text-[#A89F94]/30 mb-3" /><p className="text-[#A89F94] text-sm">No expenses logged</p></div>
              ...
              <div key={expense.id} className="px-5 py-4 hover:bg-[#EFE6DA]/50 transition-colors flex items-center justify-between group">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-[#A89F94]">{expense.category || 'Other'}{expense.vendor ? ` · ${expense.vendor}` : ''}{subMeta ? ` · Next: ${new Date(subMeta.next_due_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}` : ''}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <div className="text-right"><p className="font-semibold text-amber-600">-{formatCurrency(expense.amount)}</p><p className="text-xs text-[#A89F94]">{new Date(expense.expense_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p></div>
                  <button onClick={() => handleDeleteExpense(expense.id)} className="p-2 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-red-50 text-[#A89F94] hover:text-red-500 transition-all">
        ...
        <div className="bg-white border border-[#D8CFC4] rounded-2xl p-5 shadow-sm">
          ...
          {expenseBreakdown.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8"><PieChart size={32} className="text-[#A89F94]/30 mb-4" /><p className="text-[#A89F94] text-sm">No expense data</p></div>
          ) : (
              ...
              {expenseBreakdown.map(cat => {
                ...
                return (
                  <div key={cat.name} className="flex items-center justify-between">
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: cat.color }} /><span className="text-sm text-[#A89F94]">{cat.name}</span></div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-[#1E1E1E]/30 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white border border-[#D8CFC4] rounded-2xl p-6 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Expense Type Toggle */}
          <div>
            <label className="text-xs text-[#A89F94] mb-1.5 block">Type</label>
            <div className="flex gap-2">
              {(['one-time', 'subscription'] as const).map(t => (
                <button key={t} type="button" onClick={() => setForm(f => ({ ...f, expense_type: t }))}
                  className={`flex-1 px-3 py-2 rounded-xl text-sm font-medium border transition-all ${form.expense_type === t ? 'bg-[#B07A45]/10 text-[#B07A45] border-[#B07A45]/30' : 'bg-[#EFE6DA] text-[#A89F94] border-[#D8CFC4]'}`}>
                  {t === 'one-time' ? 'One-Time' : 'Subscription'}
                </button>
              ))}
            </div>
          </div>
          <div><label className="text-xs text-[#A89F94] mb-1.5 block">Description</label><input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} className={inputCls} /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-xs text-[#A89F94] mb-1.5 block">Amount *</label><input type="number" value={form.amount || ''} onChange={e => setForm(f => ({ ...f, amount: Number(e.target.value) }))} className={inputCls} required /></div>
            <div><label className="text-xs text-[#A89F94] mb-1.5 block">Date</label><input type="date" value={form.expense_date} onChange={e => setForm(f => ({ ...f, expense_date: e.target.value }))} className={inputCls} /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-xs text-[#A89F94] mb-1.5 block">Category</label><select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} className={inputCls}><option value="">Select...</option>{['Software & Subscriptions','Equipment & Hardware','Professional Services','Marketing & Advertising','Contractor/Freelancer','Travel & Meals','Other'].map(c => <option key={c} value={c}>{c}</option>)}</select></div>
            <div><label className="text-xs text-[#A89F94] mb-1.5 block">Vendor</label><input value={form.vendor} onChange={e => setForm(f => ({ ...f, vendor: e.target.value }))} className={inputCls} /></div>
          </div>
          {/* Subscription-specific fields */}
          {form.expense_type === 'subscription' && (
            <>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-xs text-[#A89F94] mb-1.5 block">Company Name</label><input value={form.company_name} onChange={e => setForm(f => ({ ...f, company_name: e.target.value }))} placeholder="e.g. Vercel, OpenAI" className={inputCls} /></div>
                <div><label className="text-xs text-[#A89F94] mb-1.5 block">Billing Cycle</label>
              ...
              <div><label className="text-xs text-[#A89F94] mb-1.5 block">Next Due Date</label><input type="date" value={form.next_due_date} onChange={e => setForm(f => ({ ...f, next_due_date: e.target.value }))} className={inputCls} /></div>
            </>
          )}
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving || !form.amount} className="flex-1 px-4 py-3 rounded-xl bg-[#B07A45] text-white font-medium text-sm disabled:opacity-50 flex items-center justify-center gap-2">
