...
const COLUMNS: { key: Column; label: string; icon: typeof Clock; color: string; bg: string }[] = [
  { key: 'todo', label: 'To Do', icon: CircleDot, color: 'text-[#A89F94]', bg: 'bg-[#A89F94]/10' },
...
const PRIORITY_CFG: Record<Priority, { color: string; bg: string }> = {
  low: { color: 'text-[#A89F94]', bg: 'bg-[#EFE6DA]' },
...
function getDueDateColor(due_date?: string): string {
  if (!due_date) return 'text-[#A89F94]';
...
export default function TasksPage() {
  ...
  const inputCls = 'w-full bg-[#F5EFE7] border border-[#D8CFC4] rounded-xl px-4 py-2.5 text-sm text-[#1E1E1E] focus:outline-none focus:border-[#B07A45]/50';

  return (
    <div className="space-y-6 pb-12">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#1E1E1E] flex items-center gap-3"><div className="p-2 bg-[#B07A45]/10 rounded-xl"><Layers className="w-6 h-6 text-[#B07A45]" /></div>Tasks</h1>
          <p className="text-[#A89F94] mt-1 text-sm">Manage your team&apos;s tasks</p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 text-sm text-[#A89F94]"><span>{filteredTasks.length} total</span><span>|</span><span className="text-emerald-600">{filteredTasks.filter(t => t.column === 'done').length} done</span></div>
          <div className="flex bg-white border border-[#D8CFC4] rounded-xl p-1">
            <button onClick={() => setViewMode('kanban')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${viewMode === 'kanban' ? 'bg-[#B07A45]/10 text-[#B07A45]' : 'text-[#A89F94]'}`}><LayoutGrid size={13} /> Kanban</button>
            <button onClick={() => setViewMode('list')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${viewMode === 'list' ? 'bg-[#B07A45]/10 text-[#B07A45]' : 'text-[#A89F94]'}`}><List size={13} /> List</button>
          </div>
          <select value={projectFilter} onChange={e => setProjectFilter(e.target.value)} className="bg-white border border-[#D8CFC4] rounded-xl px-3 py-1.5 text-xs text-[#1E1E1E] focus:outline-none focus:border-[#B07A45]/50">
            <option value="all">All Projects</option>
            {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
          <button onClick={() => { resetForm(); setShowForm(true); }} className="flex items-center gap-2 px-5 py-2.5 bg-[#B07A45] hover:bg-[#A67A4B] text-white rounded-xl font-medium transition-colors text-sm"><Plus className="w-4 h-4" /> New Task</button>
        </div>
      </motion.div>
            ...
            return (
              <div key={col.key} onDragOver={e => e.preventDefault()} onDrop={() => handleDrop(col.key)}
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs text-[#A89F94] bg-[#EFE6DA] px-2 py-0.5 rounded-full">{colTasks.length}</span>
                </div>
                ...
                {quickAdd?.col === col.key ? (
                  <div className="mb-3">
                    <input autoFocus value={quickAdd.title} onChange={e => setQuickAdd({ ...quickAdd, title: e.target.value })}
                      ...
                      placeholder="Task title..." className="w-full bg-[#F5EFE7] border border-[#B07A45]/30 rounded-lg px-3 py-2 text-sm text-[#1E1E1E] focus:outline-none" />
                    <div className="flex gap-2 mt-1.5">
                      <button onClick={() => handleQuickAdd(col.key, quickAdd.title)} className="px-3 py-1 bg-[#B07A45] text-white rounded-lg text-xs">Add</button>
                      <button onClick={() => setQuickAdd(null)} className="px-3 py-1 text-[#A89F94] text-xs">Cancel</button>
                    </div>
                  </div>
                ) : (
                  <button onClick={() => setQuickAdd({ col: col.key, title: '' })} className="w-full mb-3 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border border-dashed border-[#D8CFC4] text-[#A89F94] hover:border-[#B07A45]/30 hover:text-[#B07A45] text-xs transition-colors">
                ...
                <AnimatePresence mode="popLayout">
                  {colTasks.length === 0 ? (
                    <div className="text-center py-8 text-[#A89F94]/50 text-sm">No tasks</div>
                  ) : (
                      ...
                      {colTasks.map(task => {
                        ...
                        return (
                          <motion.div key={task.id} layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                            ...
                            className="bg-[#F5EFE7] border border-[#D8CFC4] rounded-xl p-3.5 cursor-grab active:cursor-grabbing hover:border-[#B07A45]/30 transition-all group">
                            <div className="flex items-start justify-between mb-2">
                              <p className="text-sm font-medium text-[#1E1E1E] leading-snug flex-1 mr-2">{task.title}</p>
                              <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => startEdit(task)} className="p-1 text-[#A89F94] hover:text-[#B07A45] rounded"><Tag className="w-3 h-3" /></button>
                                <button onClick={() => handleDelete(task.id)} className="p-1 text-[#A89F94] hover:text-red-500 rounded">
                                  {deleting === task.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />}
                                </button>
                              </div>
                            </div>
                            {task.description && <p className="text-xs text-[#A89F94] mb-2.5 line-clamp-2">{task.description}</p>}
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                {proj && <span className="text-[10px] text-[#A89F94] bg-[#EFE6DA] px-1.5 py-0.5 rounded truncate max-w-[80px]">{proj.name}</span>}
                              </div>
                              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => moveTask(task.id, 'left')} className="p-1 text-[#A89F94] hover:text-[#1E1E1E] rounded"><ArrowRight className="w-3 h-3 rotate-180" /></button>
                                <button onClick={() => moveTask(task.id, 'right')} className="p-1 text-[#A89F94] hover:text-[#1E1E1E] rounded"><ArrowRight className="w-3 h-3" /></button>
                              </div>
                            </div>
              ...
              {filteredTasks.map(task => {
                ...
                return (
                  <tr key={task.id} className="border-b border-[#D8CFC4]/50 hover:bg-[#EFE6DA]/50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-xs text-[#A89F94]">{proj?.name || '-'}</td>
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => startEdit(task)} className="p-1.5 rounded-lg text-[#A89F94] hover:text-[#B07A45]"><Tag size={14} /></button>
                        <button onClick={() => handleDelete(task.id)} className="p-1.5 rounded-lg text-[#A89F94] hover:text-red-500">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-[#1E1E1E]/30 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={resetForm}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white border border-[#D8CFC4] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-4">
                <div><label className="block text-xs text-[#A89F94] mb-1.5">Title *</label><input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} className={inputCls} placeholder="Task title" /></div>
                <div><label className="block text-xs text-[#A89F94] mb-1.5">Description</label><textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={3} className={inputCls + ' resize-none'} /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-xs text-[#A89F94] mb-1.5">Column</label><select value={form.column} onChange={e => setForm(f => ({ ...f, column: e.target.value as Column }))} className={inputCls}>{COLUMNS.map(c => <option key={c.key} value={c.key}>{c.label}</option>)}</select></div>
                  <div><label className="block text-xs text-[#A89F94] mb-1.5">Priority</label><select value={form.priority} onChange={e => setForm(f => ({ ...f, priority: e.target.value as Priority }))} className={inputCls}>{(['low','medium','high','urgent'] as Priority[]).map(p => <option key={p} value={p}>{p.charAt(0).toUpperCase()+p.slice(1)}</option>)}</select></div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div><label className="block text-xs text-[#A89F94] mb-1.5">Project</label><select value={form.project_id} onChange={e => setForm(f => ({ ...f, project_id: e.target.value }))} className={inputCls}><option value="">None</option>{projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
                </div>
              </div>
              <div className="flex justify-end gap-3 mt-8">
                <button onClick={resetForm} className="px-5 py-2.5 text-[#A89F94] hover:text-[#1E1E1E]">Cancel</button>
                <button onClick={handleSubmit} disabled={!form.title} className="px-6 py-2.5 bg-[#B07A45] hover:bg-[#A67A4B] disabled:opacity-40 text-white rounded-xl font-medium transition-colors">{editId ? 'Update' : 'Create'}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
