...
function getDueDateColor(due_date?: string): string {
  if (!due_date) return 'text-[#8C857C]';
    ...
    <div className="space-y-6 pb-12">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#8C857C] mt-1 text-sm">Manage your team&apos;s tasks</p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 text-sm text-[#8C857C]"><span>{filteredTasks.length} total</span><span>|</span><span className="text-emerald-600">{filteredTasks.filter(t => t.column === 'done').length} done</span></div>
          <div className="flex bg-white border border-[#E8E2DA] rounded-xl p-1">
            <button onClick={() => setViewMode('kanban')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${viewMode === 'kanban' ? 'bg-[#B8895A]/10 text-[#B8895A]' : 'text-[#8C857C]'}`}><LayoutGrid size={13} /> Kanban</button>
            <button onClick={() => setViewMode('list')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${viewMode === 'list' ? 'bg-[#B8895A]/10 text-[#B8895A]' : 'text-[#8C857C]'}`}><List size={13} /> List</button>
          </div>
          <select value={projectFilter} onChange={e => setProjectFilter(e.target.value)} className="bg-white border border-[#E8E2DA] rounded-xl px-3 py-1.5 text-xs text-[#2D2A26] focus:outline-none focus:border-[#B8895A]/50">
            <option value="all">All Projects</option>
            {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
          <button onClick={() => { resetForm(); setShowForm(true); }} className="flex items-center gap-2 px-5 py-2.5 bg-[#B8895A] hover:bg-[#A67A4B] text-white rounded-xl font-medium transition-colors text-sm"><Plus className="w-4 h-4" /> New Task</button>
        </div>
      </motion.div>
                ...
                {quickAdd?.col === col.key ? (
                  <div className="mb-3">
                    <input autoFocus value={quickAdd.title} onChange={e => setQuickAdd({ ...quickAdd, title: e.target.value })}
                      ...
                      placeholder="Task title..." className="w-full bg-[#FAFAFA] border border-[#B8895A]/30 rounded-lg px-3 py-2 text-sm text-[#2D2A26] focus:outline-none" />
                    <div className="flex gap-2 mt-1.5">
                      <button onClick={() => handleQuickAdd(col.key, quickAdd.title)} className="px-3 py-1 bg-[#B8895A] text-white rounded-lg text-xs">Add</button>
                      <button onClick={() => setQuickAdd(null)} className="px-3 py-1 text-[#8C857C] text-xs">Cancel</button>
                    </div>
                  </div>
                ) : (
                  <button onClick={() => setQuickAdd({ col: col.key, title: '' })} className="w-full mb-3 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border border-dashed border-[#E8E2DA] text-[#8C857C] hover:border-[#B8895A]/30 hover:text-[#B8895A] text-xs transition-colors">
                          ...
                          <motion.div key={task.id} layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                {proj && <span className="text-[10px] text-[#8C857C] bg-[#F5F0EB] px-1.5 py-0.5 rounded truncate max-w-[80px]">{proj.name}</span>}
                              </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredTasks.map(task => {
                ...
                const proj = projects.find(p => p.id === task.project_id);
                return (
                  <tr key={task.id} className="border-b border-[#E8E2DA]/50 hover:bg-[#F5F0EB]/50 transition-colors">
                    <td className="px-5 py-3.5 text-[#2D2A26] font-medium">{task.title}</td>
                    ...
                    <td className="px-5 py-3.5 text-xs text-[#8C857C]">{proj?.name || '-'}</td>
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => startEdit(task)} className="p-1.5 rounded-lg text-[#8C857C] hover:text-[#B8895A]"><Tag size={14} /></button>
                        <button onClick={() => handleDelete(task.id)} className="p-1.5 rounded-lg text-[#8C857C] hover:text-red-500">
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white border border-[#E8E2DA] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div><label className="block text-xs text-[#8C857C] mb-1.5">Project</label><select value={form.project_id} onChange={e => setForm(f => ({ ...f, project_id: e.target.value }))} className={inputCls}><option value="">None</option>{projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
                </div>
              </div>
