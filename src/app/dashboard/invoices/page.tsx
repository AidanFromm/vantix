...
export default function InvoicesPage() {
  ...
  const handlePrint = (inv: Invoice) => {
    ...
    if (!w) return;
    w.document.write(`<html><head><title>Invoice ${inv.invoice_number || inv.id.slice(0,8)}</title><style>body{font-family:system-ui;padding:40px;color:#3D2E1C}h1{color:#8B6D47}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{padding:8px 12px;text-align:left;border-bottom:1px solid #EDE8E1}.total{font-size:24px;font-weight:bold}</style></head><body>`);
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div><h1 className="text-2xl font-bold text-[#3D2E1C]">Invoices</h1><p className="text-sm text-[#A39484] mt-0.5">Manage invoices and track payments</p></div>
        <button onClick={() => { setEditingInvoice(null); setShowForm(true); }} className="flex items-center gap-2 px-4 py-2.5 bg-[#8B6D47] hover:bg-[#A67A4B] text-white rounded-xl font-medium text-sm transition-colors"><Plus size={16} /> New Invoice</button>
      </div>
        ...
        {statCards.map(s => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`rounded-2xl bg-white border ${s.border} p-4 shadow-[4px_4px_12px_#d1cdc7,-4px_-4px_12px_#ffffff]`}>
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-xs text-[#A39484]">{s.label}</p><p className="text-lg font-bold text-[#3D2E1C]">{s.value}</p></div>
            </div>
          </motion.div>
        ))}
      </div>
        ...
        <>
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by client or invoice number..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-[#EDE8E1] rounded-xl text-sm text-[#3D2E1C] focus:outline-none focus:border-[#8B6D47]/50" />
            </div>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16">
              <Receipt size={40} className="mx-auto mb-3 text-[#A39484]/30" />
              <p className="text-sm text-[#A39484]">{search || statusFilter !== 'all' ? 'No invoices found' : 'No invoices yet'}</p>
              {!search && statusFilter === 'all' && <button onClick={() => { setEditingInvoice(null); setShowForm(true); }} className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#8B6D47]/10 text-[#8B6D47] rounded-xl text-sm font-medium"><Plus size={14} /> Create Your First Invoice</button>}
            </div>
          ) : (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filtered.map(inv => {
                      const amt = inv.total || inv.amount || 0;
                      return (
                        <tr key={inv.id} className="border-b border-[#EDE8E1]/50 hover:bg-[#F5F1EC]/50 transition-colors">
                          <td className="px-5 py-3.5 font-mono text-[#8B6D47] font-medium">{inv.invoice_number || inv.id.slice(0, 8)}</td>
                          <td className="px-5 py-3.5 text-[#3D2E1C]">{inv.client?.name || 'N/A'}</td>
                          <td className="px-5 py-3.5"><StatusBadge status={inv.status} /></td>
                          <td className="px-5 py-3.5 text-right text-[#3D2E1C] font-medium">{formatCurrency(amt)}</td>
                          <td className="px-5 py-3.5 text-[#A39484]">{inv.due_date ? formatDate(inv.due_date) : 'N/A'}</td>
                          <td className="px-5 py-3.5 text-right">
                            <div className="flex items-center justify-end gap-1">
                              ...
                              <button onClick={() => handlePrint(inv)} className="p-1.5 rounded-lg text-[#A39484] hover:text-[#8B6D47] hover:bg-[#8B6D47]/10" title="Print/PDF"><Printer size={14} /></button>
                              <button onClick={() => { setEditingInvoice(inv); setShowForm(true); }} className="p-1.5 rounded-lg text-[#A39484] hover:text-[#8B6D47] hover:bg-[#8B6D47]/10"><Edit3 size={14} /></button>
                              <button onClick={() => handleDelete(inv.id)} disabled={deleting === inv.id} className="p-1.5 rounded-lg text-[#A39484] hover:text-red-500 hover:bg-red-50">
...
function InvoiceFormModal({ invoice, clients, projects, onClose, onSave }: { invoice: Invoice | null; clients: Client[]; projects: Project[]; onClose: () => void; onSave: (data: Partial<Invoice>) => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-[#3D2E1C]/30 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white border border-[#EDE8E1] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-[#EDE8E1] flex items-center justify-between">
          <h2 className="text-lg font-semibold text-[#3D2E1C]">{invoice ? 'Edit Invoice' : 'New Invoice'}</h2>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-[#F5F1EC] text-[#A39484]"><X size={18} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs text-[#A39484] mb-1.5">Invoice Number</label><input value={form.invoice_number} onChange={e => setForm(f => ({ ...f, invoice_number: e.target.value }))} placeholder="INV-1001" className={inputCls} /></div>
            <div><label className="block text-xs text-[#A39484] mb-1.5">Amount ($) *</label><input type="number" min={0} step={0.01} value={form.total || ''} onChange={e => setForm(f => ({ ...f, total: Number(e.target.value) }))} className={inputCls} required /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs text-[#A39484] mb-1.5">Client</label><select value={form.client_id} onChange={e => setForm(f => ({ ...f, client_id: e.target.value }))} className={inputCls}><option value="">Select...</option>{clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
            <div><label className="block text-xs text-[#A39484] mb-1.5">Project</label><select value={form.project_id} onChange={e => setForm(f => ({ ...f, project_id: e.target.value }))} className={inputCls}><option value="">Select...</option>{projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
          </div>
          <div><label className="block text-xs text-[#A39484] mb-1.5">Status</label>
            <div className="flex flex-wrap gap-2">
              {(['draft','sent','paid','overdue','cancelled'] as const).map(s => {
                const cfg = statusConfig[s];
                return <button key={s} type="button" onClick={() => setForm(f => ({ ...f, status: s, ...(s === 'paid' && !f.paid_at ? { paid_at: new Date().toISOString().split('T')[0] } : {}) }))} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${form.status === s ? `${cfg.bg} ${cfg.color} ${cfg.border}` : 'bg-[#F5F1EC] text-[#A39484] border-[#EDE8E1]'}`}>{cfg.label}</button>;
              })}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="block text-xs text-[#A39484] mb-1.5">Paid Date</label><input type="date" value={form.paid_at} onChange={e => setForm(f => ({ ...f, paid_at: e.target.value }))} className={inputCls} /></div>
          </div>
          <div><label className="block text-xs text-[#A39484] mb-1.5">Notes</label><textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={3} className={inputCls + ' resize-none'} /></div>
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving || !form.total} className="flex-1 px-4 py-3 rounded-xl bg-[#8B6D47] text-white font-medium text-sm disabled:opacity-50 flex items-center justify-center gap-2">
