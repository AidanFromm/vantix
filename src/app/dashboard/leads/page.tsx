      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="relative w-full max-w-lg bg-white border border-[#E8E2DA] rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div><label className="text-xs text-[#8C857C] mb-1.5 block">Status</label>
            <div className="flex flex-wrap gap-2">{ALL_STATUSES.map(s => <button key={s.id} type="button" onClick={() => setForm(p => ({ ...p, status: s.id }))} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${form.status === s.id ? `${s.bg} ${s.color} ${s.border}` : 'bg-[#F5F0EB] text-[#8C857C] border-[#E8E2DA]'}`}>{s.label}</button>)}</div>
          </div>
          <div><label className="text-xs text-[#8C857C] mb-1.5 block">Tags</label><input type="text" value={form.tags} onChange={e => setForm(p => ({ ...p, tags: e.target.value }))} className={inputCls} placeholder="tag1, tag2" /></div>
...
function KanbanCard({ lead, onSelect, onDelete, deleting }: { lead: Lead; onSelect: (l: Lead) => void; onDelete: (id: string) => void; deleting: string | null }) {
  ...
  return (
    <div draggable onDragStart={e => { e.dataTransfer.setData('leadId', lead.id); e.dataTransfer.effectAllowed = 'move'; }}>
      <div className="group cursor-pointer rounded-xl bg-white border border-[#E8E2DA] hover:border-[#B8895A]/40 p-3.5 transition-all shadow-sm" onClick={() => onSelect(lead)}>
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-[#2D2A26] text-sm truncate group-hover:text-[#B8895A]">{lead.company || lead.name}</h3>
            <p className="text-xs text-[#8C857C] truncate mt-0.5">{lead.name}</p>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-[10px] text-[#8C857C]"><Clock size={10} />{daysInStage}d in stage</div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {leads.map(lead => {
              ...
              return (
                <tr key={lead.id} className="border-b border-[#E8E2DA]/50 hover:bg-[#F5F0EB]/50 transition-colors cursor-pointer" onClick={() => onSelect(lead)}>
                  <td className="px-5 py-3.5 font-medium text-[#2D2A26]">{lead.name}</td>
                  <td className="px-5 py-3.5 text-[#8C857C]">{lead.company || '-'}</td>
                  ...
                  <td className="px-5 py-3.5 text-right text-[#B8895A] font-medium">{lead.estimated_value ? formatCurrency(lead.estimated_value) : '-'}</td>
                  <td className="px-5 py-3.5 text-right"><ScoreBadge score={lead.score} /></td>
                  <td className="px-5 py-3.5 text-[#8C857C]">{formatDate(lead.created_at)}</td>
                  <td className="px-5 py-3.5 text-right">
                    <button onClick={e => { e.stopPropagation(); onDelete(lead.id); }} className="p-1.5 rounded-lg text-[#8C857C] hover:text-red-500 hover:bg-red-50">
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div className="flex bg-white border border-[#E8E2DA] rounded-xl p-1">
          <button onClick={() => setViewMode('kanban')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === 'kanban' ? 'bg-[#B8895A]/10 text-[#B8895A]' : 'text-[#8C857C]'}`}><LayoutGrid size={15} /> Kanban</button>
          <button onClick={() => setViewMode('list')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === 'list' ? 'bg-[#B8895A]/10 text-[#B8895A]' : 'text-[#8C857C]'}`}><List size={15} /> List</button>
          <button onClick={() => setViewMode('scraped')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === 'scraped' ? 'bg-[#B8895A]/10 text-[#B8895A]' : 'text-[#8C857C]'}`}><Download size={15} /> Scraped ({scrapedLeads.length})</button>
        </div>
        {viewMode !== 'scraped' && (
      ...
      {viewMode === 'kanban' ? (
        ...
        filtered.length === 0 ? (
          <div className="text-center py-16"><p className="text-sm text-[#8C857C]">No leads found</p></div>
        ) : (
          <ListView leads={filtered} onSelect={l => { setEditingLead(l); setShowModal(true); }} onDelete={handleDelete} deleting={deleting} />
        )
      ) : (
        scrapedLeads.length === 0 ? (
          <div className="text-center py-16"><div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-[#B8895A]/10 flex items-center justify-center"><Search size={28} className="text-[#B8895A]" /></div><h3 className="font-semibold text-[#2D2A26] mb-2">No scraped leads yet</h3></div>
        ) : (
