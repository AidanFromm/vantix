...
function LeadModal({ lead, onClose, onSave }: { lead?: Lead | null; onClose: () => void; onSave: (data: Partial<Lead>) => void }) {
  ...
  const inputCls = 'w-full bg-[#FBF9F6] border border-[#EDE8E1] rounded-xl px-4 py-3 text-sm text-[#3D2E1C] focus:outline-none focus:border-[#8B6D47]/50';
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="relative w-full max-w-lg bg-white border border-[#EDE8E1] rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-[#EDE8E1] flex items-center justify-between">
          <h2 className="text-lg font-semibold text-[#3D2E1C]">{lead ? 'Edit Lead' : 'Add New Lead'}</h2>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-[#F5F1EC] text-[#A39484]"><X size={18} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div><label className="text-xs text-[#A39484] mb-1.5 block">Name *</label><input type="text" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required className={inputCls} /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-xs text-[#A39484] mb-1.5 block">Email</label><input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} className={inputCls} /></div>
            <div><label className="text-xs text-[#A39484] mb-1.5 block">Phone</label><input type="tel" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} className={inputCls} /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-xs text-[#A39484] mb-1.5 block">Company</label><input type="text" value={form.company} onChange={e => setForm(p => ({ ...p, company: e.target.value }))} className={inputCls} /></div>
            <div><label className="text-xs text-[#A39484] mb-1.5 block">Source</label><input type="text" value={form.source} onChange={e => setForm(p => ({ ...p, source: e.target.value }))} className={inputCls} /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-xs text-[#A39484] mb-1.5 block">Estimated Value</label><input type="number" value={form.estimated_value} onChange={e => setForm(p => ({ ...p, estimated_value: parseFloat(e.target.value) || 0 }))} className={inputCls} /></div>
            <div><label className="text-xs text-[#A39484] mb-1.5 block">Score (0-100)</label><input type="number" min={0} max={100} value={form.score} onChange={e => setForm(p => ({ ...p, score: parseInt(e.target.value) || 0 }))} className={inputCls} /></div>
          </div>
          <div><label className="text-xs text-[#A39484] mb-1.5 block">Status</label>
            <div className="flex flex-wrap gap-2">{ALL_STATUSES.map(s => <button key={s.id} type="button" onClick={() => setForm(p => ({ ...p, status: s.id }))} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${form.status === s.id ? `${s.bg} ${s.color} ${s.border}` : 'bg-[#F5F1EC] text-[#A39484] border-[#EDE8E1]'}`}>{s.label}</button>)}</div>
          </div>
          <div><label className="text-xs text-[#A39484] mb-1.5 block">Tags</label><input type="text" value={form.tags} onChange={e => setForm(p => ({ ...p, tags: e.target.value }))} className={inputCls} placeholder="tag1, tag2" /></div>
          <div><label className="text-xs text-[#A39484] mb-1.5 block">Notes</label><textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} rows={3} className={inputCls + ' resize-none'} /></div>
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving || !form.name.trim()} className="flex-1 px-4 py-3 rounded-xl bg-[#8B6D47] text-white font-medium text-sm disabled:opacity-50 flex items-center justify-center gap-2">
...
function KanbanCard({ lead, onSelect, onDelete, deleting }: { lead: Lead; onSelect: (l: Lead) => void; onDelete: (id: string) => void; deleting: string | null }) {
  ...
  return (
    <div draggable onDragStart={e => { e.dataTransfer.setData('leadId', lead.id); e.dataTransfer.effectAllowed = 'move'; }}>
      <div className="group cursor-pointer rounded-xl bg-white border border-[#EDE8E1] hover:border-[#8B6D47]/40 p-3.5 transition-all shadow-sm" onClick={() => onSelect(lead)}>
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-[#3D2E1C] text-sm truncate group-hover:text-[#8B6D47]">{lead.company || lead.name}</h3>
            <p className="text-xs text-[#A39484] truncate mt-0.5">{lead.name}</p>
          </div>
          <ScoreBadge score={lead.score || 0} />
        </div>
        {lead.estimated_value ? <div className="flex items-center gap-1 text-[#8B6D47] mb-2"><DollarSign size={14} /><span className="font-bold text-sm">{formatCurrency(lead.estimated_value)}</span></div> : null}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-[10px] text-[#A39484]"><Clock size={10} />{daysInStage}d in stage</div>
          <button onClick={e => { e.stopPropagation(); onDelete(lead.id); }} className="p-1 rounded text-[#A39484] hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all">
...
function KanbanColumn({ status, leads, onSelect, onDrop, onDelete, deleting, onConvertToClient }: {
  ...
  return (
    <div className="flex flex-col min-w-[260px] w-[260px] flex-shrink-0 lg:min-w-0 lg:w-auto lg:flex-1 h-full">
      <div className={`px-3 py-3 rounded-t-xl border border-b-0 bg-white transition-all ${isDragOver ? 'border-[#8B6D47]/50 bg-[#8B6D47]/5' : 'border-[#EDE8E1]'}`}>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2"><h3 className={`font-semibold text-sm ${status.color}`}>{status.label}</h3><span className="text-xs px-1.5 py-0.5 rounded-md bg-[#F5F1EC] text-[#A39484] font-medium">{leads.length}</span></div>
        </div>
        <div className="text-xs text-[#A39484]">Total: <span className="text-[#8B6D47] font-semibold">{formatCurrency(totalValue)}</span></div>
      </div>
      <div
        ...
        className={`flex-1 p-2.5 rounded-b-xl border border-t-0 min-h-[300px] max-h-[calc(100vh-400px)] overflow-y-auto transition-all ${isDragOver ? 'bg-[#8B6D47]/5 border-[#8B6D47]/40' : 'border-[#EDE8E1]'}`}
      >
        <div className="flex flex-col gap-2.5">
          ...
          {leads.length === 0 && <div className="py-10 text-center"><Users size={18} className="mx-auto mb-2 text-[#A39484]/40" /><p className="text-xs text-[#A39484]">No leads here</p></div>}
        </div>
      </div>
    </div>
  );
...
function ListView({ leads, onSelect, onDelete, deleting }: { leads: Lead[]; onSelect: (l: Lead) => void; onDelete: (id: string) => void; deleting: string | null }) {
  ...
  return (
    <div className="rounded-2xl bg-white border border-[#EDE8E1] overflow-hidden shadow-[4px_4px_12px_#d1cdc7,-4px_-4px_12px_#ffffff]">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-[#EDE8E1] text-[#A39484] text-xs uppercase tracking-wider">
          ...
          <tbody>
            {leads.map(lead => {
              ...
              return (
                <tr key={lead.id} className="border-b border-[#EDE8E1]/50 hover:bg-[#F5F1EC]/50 transition-colors cursor-pointer" onClick={() => onSelect(lead)}>
                  <td className="px-5 py-3.5 font-medium text-[#3D2E1C]">{lead.name}</td>
                  <td className="px-5 py-3.5 text-[#A39484]">{lead.company || '-'}</td>
                  ...
                  <td className="px-5 py-3.5 text-right text-[#8B6D47] font-medium">{lead.estimated_value ? formatCurrency(lead.estimated_value) : '-'}</td>
                  <td className="px-5 py-3.5 text-right"><ScoreBadge score={lead.score} /></td>
                  <td className="px-5 py-3.5 text-[#A39484]">{formatDate(lead.created_at)}</td>
                  <td className="px-5 py-3.5 text-right">
                    <button onClick={e => { e.stopPropagation(); onDelete(lead.id); }} className="p-1.5 rounded-lg text-[#A39484] hover:text-red-500 hover:bg-red-50">
...
export default function LeadsPage() {
  ...
  if (loading) return <div className="space-y-6 animate-pulse"><div className="h-8 w-48 bg-[#EDE8E1] rounded-lg" /><div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{[...Array(4)].map((_, i) => <div key={i} className="h-24 bg-[#EDE8E1]/50 rounded-2xl" />)}</div></div>;

  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-2xl sm:text-3xl font-bold text-[#3D2E1C]">Leads</h1><p className="text-sm text-[#A39484] mt-1">Manage your sales pipeline</p></div>
        <button onClick={() => { setEditingLead(null); setShowModal(true); }} className="flex items-center gap-2 bg-[#8B6D47] hover:bg-[#A67A4B] text-white px-5 py-2.5 rounded-xl font-medium transition-all shadow-lg shadow-[#8B6D47]/20 text-sm"><Plus size={18} /> Add Lead</button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Pipeline Value', value: formatCurrency(stats.pipelineValue), icon: Target, color: 'text-[#8B6D47]', bg: 'bg-[#8B6D47]/10', border: 'border-[#8B6D47]/20' },
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div className="flex bg-white border border-[#EDE8E1] rounded-xl p-1">
          <button onClick={() => setViewMode('kanban')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === 'kanban' ? 'bg-[#8B6D47]/10 text-[#8B6D47]' : 'text-[#A39484]'}`}><LayoutGrid size={15} /> Kanban</button>
          <button onClick={() => setViewMode('list')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === 'list' ? 'bg-[#8B6D47]/10 text-[#8B6D47]' : 'text-[#A39484]'}`}><List size={15} /> List</button>
          <button onClick={() => setViewMode('scraped')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode === 'scraped' ? 'bg-[#8B6D47]/10 text-[#8B6D47]' : 'text-[#A39484]'}`}><Download size={15} /> Scraped ({scrapedLeads.length})</button>
        </div>
        {viewMode !== 'scraped' && (
          <div className="relative flex-1 sm:max-w-xs">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#A39484]" />
            <input type="text" placeholder="Search leads..." value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white border border-[#EDE8E1] rounded-xl pl-10 pr-4 py-2 text-sm text-[#3D2E1C] focus:outline-none focus:border-[#8B6D47]/50" />
          </div>
        )}
      </div>
      ...
      {viewMode === 'kanban' ? (
        filtered.length === 0 && !search ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-20 h-20 rounded-2xl bg-[#8B6D47]/10 flex items-center justify-center mb-6"><Target size={40} className="text-[#8B6D47]" /></div>
            <h3 className="text-xl font-semibold text-[#3D2E1C] mb-2">No leads yet</h3>
            <p className="text-[#A39484] max-w-md mb-6">Start building your sales pipeline by adding leads.</p>
            <button onClick={() => { setEditingLead(null); setShowModal(true); }} className="flex items-center gap-2 px-5 py-3 rounded-xl bg-[#8B6D47]/10 text-[#8B6D47] border border-[#8B6D47]/30 font-medium"><Plus size={18} /> Add Your First Lead</button>
          </div>
        ) : (
        ...
        filtered.length === 0 ? (
          <div className="text-center py-16"><p className="text-sm text-[#A39484]">No leads found</p></div>
        ) : (
          <ListView leads={filtered} onSelect={l => { setEditingLead(l); setShowModal(true); }} onDelete={handleDelete} deleting={deleting} />
        )
      ) : (
        scrapedLeads.length === 0 ? (
          <div className="text-center py-16"><div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-[#8B6D47]/10 flex items-center justify-center"><Search size={28} className="text-[#8B6D47]" /></div><h3 className="font-semibold text-[#3D2E1C] mb-2">No scraped leads yet</h3></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {scrapedLeads.map((s, i) => (
              <motion.div key={s.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="p-4 rounded-xl bg-white border border-[#EDE8E1] hover:border-[#8B6D47]/30 transition-all shadow-sm">
                <h4 className="text-sm font-semibold text-[#3D2E1C] truncate mb-2">{s.business_name || 'Unknown Business'}</h4>
                <div className="space-y-1 mb-3">
                  {s.email && <p className="text-xs text-[#A39484] flex items-center gap-1.5"><Mail size={11} />{s.email}</p>}
                  {s.phone && <p className="text-xs text-[#A39484] flex items-center gap-1.5"><Phone size={11} />{s.phone}</p>}
                </div>
                {s.status !== 'added_to_leads' && s.status !== 'ignored' && (
                  <button onClick={() => handleConvertScraped(s)} className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-[#8B6D47]/10 text-[#8B6D47] hover:bg-[#8B6D47]/20 text-xs font-medium"><Plus size={12} /> Convert to Lead</button>
                )}
              </motion.div>
            ))}
          </div>
