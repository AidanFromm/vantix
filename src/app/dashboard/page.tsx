          ...
          projects.slice(0, 5).map((project, i) => {
            ...
            return (
              <motion.div key={project.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 + i * 0.1 }} className="group p-4 rounded-xl bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 hover:border-white/10 transition-all cursor-pointer">
                <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${colors.dot}`} /><span className="text-sm font-medium text-white group-hover:text-emerald-400 transition-colors">{project.name}</span></div><span className="text-sm font-semibold text-white">{project.progress ?? (project.budget ? Math.round(((project.paid || 0) / project.budget) * 100) : 0)}%</span></div>
                <div className="relative h-2 bg-white/5 rounded-full overflow-hidden"><motion.div className={`absolute inset-y-0 left-0 rounded-full ${colors.bar}`} initial={{ width: 0 }} animate={{ width: `${project.progress ?? (project.budget ? Math.round(((project.paid || 0) / project.budget) * 100) : 0)}%` }} transition={{ duration: 1, delay: 0.8 + i * 0.1, ease: 'easeOut' }} /></div>
                <div className="flex items-center justify-between mt-2"><p className="text-xs text-gray-500">{project.client?.name || 'No client'}</p>{project.budget ? <p className="text-xs text-emerald-400">${(project.spent || project.paid || 0).toLocaleString()} / ${project.budget.toLocaleString()}</p> : null}</div>
              </motion.div>
            );
          })
        )}
