...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-8 max-w-4xl pb-12">
      <div><h1 className="text-2xl sm:text-3xl font-bold text-[#1E1E1E]">Settings</h1><p className="text-[#7A746C] mt-1 text-sm">Manage integrations and team access</p></div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white border border-[#D8CFC4] rounded-2xl p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-6"><div className="w-10 h-10 rounded-xl bg-[#9B6C3C]/10 flex items-center justify-center"><User className="text-[#9B6C3C]" size={20} /></div><div><h2 className="text-xl font-semibold text-[#1E1E1E]">Your Profile</h2><p className="text-sm text-[#7A746C]">Logged in as {user?.name || 'Unknown'}</p></div></div>
        <div className="grid md:grid-cols-3 gap-4">
          {[{ label: 'Name', value: user?.name }, { label: 'Email', value: user?.email }, { label: 'Role', value: user?.role }].map(f => (
            <div key={f.label} className="p-4 rounded-xl bg-[#F5EFE7] border border-[#D8CFC4]"><p className="text-xs text-[#7A746C] mb-1">{f.label}</p><p className="font-medium text-[#1E1E1E] capitalize">{f.value || '-'}</p></div>
          ))}
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white border border-[#D8CFC4] rounded-2xl p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 rounded-xl bg-[#9B6C3C]/10 flex items-center justify-center"><Zap className="text-[#9B6C3C]" size={20} /></div><div><h2 className="text-xl font-semibold text-[#1E1E1E]">Integrations</h2><p className="text-sm text-[#7A746C]">Connect external services to power up Vantix</p></div></div>
        ...
        <div className="space-y-4 mt-6">
          {integrations.map(integration => {
            ...
            return (
              <div key={integration.id} className="bg-[#F5EFE7] rounded-xl border border-[#D8CFC4] overflow-hidden hover:border-[#9B6C3C]/30 transition-colors">
                <div className="p-4 cursor-pointer" onClick={() => setExpandedIntegration(isExpanded ? null : integration.id)}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div><h3 className="font-semibold text-[#1E1E1E]">{integration.name}</h3><p className="text-sm text-[#7A746C]">{integration.description}</p></div>
                    </div>
                ...
                {isExpanded && (
                  <div className="border-t border-[#D8CFC4] p-4 bg-white">
                    ...
                    <button className="mt-4 w-full bg-[#9B6C3C] hover:bg-[#A67A4B] text-white py-3 rounded-xl font-medium transition-all">Connect {integration.name}</button>
                  </div>
                )}
              </div>
            );
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white border border-[#D8CFC4] rounded-2xl p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-6"><div className="w-10 h-10 rounded-xl bg-purple-50 flex items-center justify-center"><Shield className="text-purple-500" size={20} /></div><div><h2 className="text-xl font-semibold text-[#1E1E1E]">Team Members</h2><p className="text-sm text-[#7A746C]">{teamMembers.length} members with access</p></div></div>
        <div className="grid md:grid-cols-2 gap-4">
          {teamMembers.map(member => (
            <div key={member.name} className="flex items-center gap-4 p-4 bg-[#F5EFE7] rounded-xl border border-[#D8CFC4]">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg ${member.type === 'bot' ? 'bg-[#9B6C3C]/10 text-[#9B6C3C]' : 'bg-purple-50 text-purple-500'}`}>{member.name[0]}</div>
              <div><h3 className="font-medium text-[#1E1E1E]">{member.name}</h3><p className="text-sm text-[#7A746C]">{member.role}</p></div>
            </div>
          ))}
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="flex justify-end">
        <button onClick={handleSave} className={`flex items-center gap-2 px-6 py-3 rounded-xl font-medium transition-all ${saved ? 'bg-emerald-500 text-white' : 'bg-[#9B6C3C] hover:bg-[#A67A4B] text-white'}`}><Save size={20} />{saved ? 'Saved!' : 'Save Changes'}</button>
      </motion.div>
    </div>
  );
}