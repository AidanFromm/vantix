          ...
          <div className="flex bg-[#1C1C1C] rounded-xl p-1">
            {(['overview', 'revenue', 'expenses'] as const).map(v => (
              <button key={v} onClick={() => setView(v)}
                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${view === v ? 'bg-[#B07A45]/50 text-white' : 'text-[#A39B90] hover:text-white'}`}>
                {v.charAt(0).toUpperCase() + v.slice(1)}
              </button>
            ))}
          </div>
          <button onClick={() => setShowExpenseForm(true)} className="flex items-center gap-2 px-5 py-2.5 bg-[#B07A45]/50 hover:bg-[#8E5E34] text-white rounded-xl font-medium transition-colors">
      ...
      {loading ? (
        ...
        <>
          ...
          <motion.div initial="hidden" animate="visible" variants={stagger} className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
            {[
              { label: 'Total Revenue', value: totalRevenue, icon: TrendingUp, color: 'text-[#C89A6A]', prefix: '$' },
              { label: 'Total Expenses', value: totalExpenses, icon: TrendingDown, color: 'text-[#B0614A]', prefix: '$' },
              { label: 'Net Profit', value: netProfit, icon: DollarSign, color: netProfit >= 0 ? 'text-[#C89A6A]' : 'text-[#B0614A]', prefix: '$' },
              { label: 'Outstanding', value: outstanding, icon: Receipt, color: 'text-[#C89A6A]', prefix: '$' },
            ].map((kpi, i) => (
          ...
          {(view === 'overview' || view === 'revenue') && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-[#1C1C1C] border border-[#1C1C1C] rounded-xl p-6 mb-8">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                  <BarChart3 className="w-5 h-5 text-[#C89A6A]" /> Monthly Breakdown
                </h2>
                <div className="flex items-center gap-4 text-xs">
                  <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 bg-[#B07A45]/50 rounded-full" /> Revenue</span>
                  <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 bg-[#B0614A]/50 rounded-full" /> Expenses</span>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {(view === 'overview' || view === 'expenses') && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-[#1C1C1C] border border-[#1C1C1C] rounded-xl p-6">
                <h2 className="text-lg font-semibold text-white flex items-center gap-2 mb-6">
                  <PieChart className="w-5 h-5 text-[#C89A6A]" /> Expense Categories
                </h2>
                    ...
                    {categoryBreakdown.map(([cat, amount]) => (
                      <div key={cat}>
                        <div className="flex justify-between text-sm mb-1">
                          <span className="text-[#E3D9CD]">{cat}</span>
                          <span className="text-[#A39B90]">${amount.toLocaleString()}</span>
                        </div>
                        <div className="h-2 bg-[#1C1C1C] rounded-full overflow-hidden">
                          <div className="h-full bg-[#B07A45]/50 rounded-full transition-all" style={{ width: `${(amount / totalExpenses) * 100}%` }} />
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-[#1C1C1C] border border-[#1C1C1C] rounded-xl p-6">
              <h2 className="text-lg font-semibold text-white flex items-center gap-2 mb-6">
                <CreditCard className="w-5 h-5 text-[#C89A6A]" /> Recent Transactions
              </h2>
          ...
          {/* Monthly P&L Table */}
          {view === 'overview' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-[#1C1C1C] border border-[#1C1C1C] rounded-xl p-6 mt-6">
              <h2 className="text-lg font-semibold text-white flex items-center gap-2 mb-6">
                <Calendar className="w-5 h-5 text-[#C89A6A]" /> Monthly P&L
              </h2>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {monthlyData.filter(m => m.revenue > 0 || m.expenses > 0).map((m, i) => (
                      <tr key={i} className="border-b border-[#1C1C1C]/50">
                        <td className="py-2.5 pr-4 text-white">{m.month}</td>
                        <td className="py-2.5 px-4 text-right text-[#C89A6A]">${m.revenue.toLocaleString()}</td>
                        <td className="py-2.5 px-4 text-right text-[#B0614A]">${m.expenses.toLocaleString()}</td>
                        <td className={`py-2.5 pl-4 text-right font-medium ${m.profit >= 0 ? 'text-[#C89A6A]' : 'text-[#B0614A]'}`}>${m.profit.toLocaleString()}</td>
                      </tr>
                    ))}
        ...
        {showExpenseForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowExpenseForm(false)}>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="bg-[#1C1C1C] border border-[#1C1C1C] rounded-2xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
              <h2 className="text-xl font-bold text-white mb-6">Add Expense</h2>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-[#A39B90] mb-1.5">Amount ($)</label>
                    <input type="number" value={expForm.amount || ''} onChange={e => setExpForm(f => ({ ...f, amount: Number(e.target.value) }))} className="w-full px-4 py-2.5 bg-[#1C1C1C] border border-[#1C1C1C] rounded-xl text-white placeholder-[#4B4B4B] focus:outline-none focus:border-[#B07A45]/50 transition-colors" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[#A39B90] mb-1.5">Date</label>
                    <input type="date" value={expForm.expense_date} onChange={e => setExpForm(f => ({ ...f, expense_date: e.target.value }))} className="w-full px-4 py-2.5 bg-[#1C1C1C] border border-[#1C1C1C] rounded-xl text-white focus:outline-none focus:border-[#B07A45]/50 transition-colors" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#A39B90] mb-1.5">Category</label>
                  <select value={expForm.category} onChange={e => setExpForm(f => ({ ...f, category: e.target.value }))} className="w-full px-4 py-2.5 bg-[#1C1C1C] border border-[#1C1C1C] rounded-xl text-white focus:outline-none focus:border-[#B07A45]/50 transition-colors">
                ...
                <div>
                  <label className="block text-sm font-medium text-[#A39B90] mb-1.5">Vendor</label>
                  <input value={expForm.vendor} onChange={e => setExpForm(f => ({ ...f, vendor: e.target.value }))} className="w-full px-4 py-2.5 bg-[#1C1C1C] border border-[#1C1C1C] rounded-xl text-white placeholder-[#4B4B4B] focus:outline-none focus:border-[#B07A45]/50 transition-colors" placeholder="Vercel, DigitalOcean, etc." />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#A39B90] mb-1.5">Description</label>
                  <input value={expForm.description} onChange={e => setExpForm(f => ({ ...f, description: e.target.value }))} className="w-full px-4 py-2.5 bg-[#1C1C1C] border border-[#1C1C1C] rounded-xl text-white placeholder-[#4B4B4B] focus:outline-none focus:border-[#B07A45]/50 transition-colors" placeholder="Monthly hosting" />
                </div>
              </div>
              <div className="flex justify-end gap-3 mt-8">
                <button onClick={() => setShowExpenseForm(false)} className="px-5 py-2.5 text-[#A39B90] hover:text-white transition-colors">Cancel</button>
                <button onClick={handleExpense} disabled={!expForm.amount} className="px-6 py-2.5 bg-[#B07A45]/50 hover:bg-[#8E5E34] disabled:opacity-40 text-white rounded-xl font-medium transition-colors">
                  Add Expense
                </button>
              </div>
            </motion.div>
