  ...
  return (
    <svg viewBox={`0 0 ${chartW} ${chartH + 30}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
      {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => (
        <line key={i} x1={0} x2={chartW} y1={chartH * (1 - pct)} y2={chartH * (1 - pct)} stroke="#D8CFC4" strokeWidth="0.5" />
      ))}
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            ...
            {d.value > 0 && (
              <text x={x + barW / 2} y={chartH - h - 6} textAnchor="middle" className="fill-[#1E1E1E]" fontSize="9" fontWeight="600">
...
export default function ReportsPage() {
  ...
  if (loading) return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{[...Array(4)].map((_, i) => <div key={i} className="h-28 bg-[#D8CFC4]/50 rounded-2xl" />)}</div>
    </div>
  );
    ...
    <div className="space-y-8 pb-12">
        ...
        {metrics.map((m, i) => (
          <motion.div key={m.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
            ...
            <div className="flex items-end gap-2">
              <span className="text-2xl font-bold text-[#1E1E1E]">{m.value}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          className="bg-white border border-[#D8CFC4] rounded-xl overflow-hidden shadow-sm">
          <div className="px-4 py-3 border-b border-[#D8CFC4] flex items-center gap-2">
            <div className="p-1.5 rounded-lg bg-purple-50"><DollarSign size={14} className="text-purple-600" /></div>
            <h3 className="text-sm font-semibold text-[#1E1E1E]">Expense Breakdown</h3>
          </div>
          <div className="p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
          className="bg-white border border-[#D8CFC4] rounded-xl overflow-hidden shadow-sm">
          <div className="px-4 py-3 border-b border-[#D8CFC4] flex items-center gap-3">
            <div className="p-2 rounded-xl bg-blue-50"><Briefcase size={16} className="text-blue-600" /></div>
            <h3 className="text-sm font-semibold text-[#1E1E1E]">Project Profitability</h3>
          </div>
          <div className="p-5">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
          ...
          <div className="px-4 py-3 border-b border-[#D8CFC4] flex items-center gap-3">
            ...
            <h3 className="text-sm font-semibold text-[#1E1E1E]">Lead Conversion</h3>
          </div>
          <div className="p-5">
