...
function BarChartSVG({ data, color = '#B8895A' }: { data: { label: string; value: number }[]; color?: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${chartW} ${chartH + 30}`} className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
      {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => (
        <line key={i} x1={0} x2={chartW} y1={chartH * (1 - pct)} y2={chartH * (1 - pct)} stroke="#E8E2DA" strokeWidth="0.5" />
      ))}
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            ...
            {d.value > 0 && (
              <text x={x + barW / 2} y={chartH - h - 6} textAnchor="middle" className="fill-[#2D2A26]" fontSize="9" fontWeight="600">
                ${(d.value / 1000).toFixed(d.value >= 1000 ? 1 : 0)}{d.value >= 1000 ? 'k' : ''}
              </text>
            )}
            <text x={x + barW / 2} y={chartH + 16} textAnchor="middle" className="fill-[#8C857C]" fontSize="10">{d.label}</text>
          </g>
        );
      })}
    </svg>
      ...
      {data.map((d, i) => (
        <div key={i}>
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-[#8C857C]">{d.label}</span>
            <span className="text-xs font-semibold text-[#2D2A26]">{formatCurrency(d.value)}</span>
          </div>
          <div className="h-5 bg-[#F5F0EB] rounded-lg overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
...
export default function ReportsPage() {
  ...
  const expenseBreakdown = useMemo(() => {
    ...
    const colors = ['#B8895A', '#8B5CF6', '#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#EC4899'];
  ...
  if (loading) return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{[...Array(4)].map((_, i) => <div key={i} className="h-28 bg-[#E8E2DA]/50 rounded-2xl" />)}</div>
    </div>
  );
  ...
  const metrics = [
    ...
    { label: 'Net Profit', value: formatCurrency(netProfit), icon: BarChart3, color: netProfit >= 0 ? 'text-[#B8895A]' : 'text-red-500', bg: 'bg-[#B8895A]/10', border: 'border-[#B8895A]/20', trend: netProfit >= 0 },
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-[#B8895A]/10"><BarChart3 size={22} className="text-[#B8895A]" /></div>
          <div>
            <h1 className="text-2xl font-bold text-[#2D2A26]">Reports</h1>
            <p className="text-sm text-[#8C857C]">Analytics and business insights</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Filter size={14} className="text-[#8C857C]" />
          <div className="flex bg-white border border-[#E8E2DA] rounded-xl p-1">
            {(['ytd', '3m', '6m', '12m'] as const).map(r => (
              <button key={r} onClick={() => setDateRange(r)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${dateRange === r ? 'bg-[#B8895A]/10 text-[#B8895A]' : 'text-[#8C857C] hover:text-[#2D2A26]'}`}>
        ...
        {metrics.map((m, i) => (
          <motion.div key={m.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
            ...
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs text-[#8C857C] uppercase tracking-wide">{m.label}</span>
              <div className={`p-2 rounded-xl ${m.bg}`}><m.icon size={16} className={m.color} /></div>
            </div>
            <div className="flex items-end gap-2">
              <span className="text-2xl font-bold text-[#2D2A26]">{m.value}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          className="lg:col-span-2 bg-white border border-[#E8E2DA] rounded-2xl overflow-hidden shadow-[4px_4px_12px_#d1cdc7,-4px_-4px_12px_#ffffff]">
          <div className="px-6 py-4 border-b border-[#E8E2DA] flex items-center gap-3">
            <div className="p-2 rounded-xl bg-[#B8895A]/10"><BarChart3 size={16} className="text-[#B8895A]" /></div>
            <h3 className="text-sm font-semibold text-[#2D2A26]">Revenue by Month</h3>
          </div>
          <div className="p-6">
            {revenueByMonth.some(m => m.value > 0) ? (
              <BarChartSVG data={revenueByMonth} />
            ) : (
              <div className="text-center py-12 text-sm text-[#8C857C]">No revenue data for this period</div>
            )}
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          className="bg-white border border-[#E8E2DA] rounded-2xl overflow-hidden shadow-[4px_4px_12px_#d1cdc7,-4px_-4px_12px_#ffffff]">
          <div className="px-6 py-4 border-b border-[#E8E2DA] flex items-center gap-3">
            <div className="p-2 rounded-xl bg-purple-50"><DollarSign size={16} className="text-purple-600" /></div>
            <h3 className="text-sm font-semibold text-[#2D2A26]">Expense Breakdown</h3>
          </div>
          <div className="p-5">
            {expenseBreakdown.length > 0 ? (
              <HorizontalBars data={expenseBreakdown} />
            ) : (
              <div className="text-center py-12 text-sm text-[#8C857C]">No expense data</div>
            )}
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
          ...
          <div className="px-6 py-4 border-b border-[#E8E2DA] flex items-center gap-3">
            ...
            <h3 className="text-sm font-semibold text-[#2D2A26]">Project Profitability</h3>
          </div>
          <div className="p-5">
            {projectProfit.length > 0 ? (
              <div className="space-y-3">
                {projectProfit.map((p, i) => (
                  <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-[#FAFAFA] border border-[#E8E2DA]">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[#2D2A26] truncate">{p.name}</p>
                      <p className="text-xs text-[#8C857C]">Budget: {formatCurrency(p.budget)} | Spent: {formatCurrency(p.spent)}</p>
                    </div>
              ...
              <div className="text-center py-12 text-sm text-[#8C857C]">No project data with budgets</div>
            )}
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
          ...
          <div className="px-6 py-4 border-b border-[#E8E2DA] flex items-center gap-3">
            ...
            <h3 className="text-sm font-semibold text-[#2D2A26]">Lead Conversion</h3>
          </div>
          <div className="p-5">
            <div className="grid grid-cols-2 gap-4 mb-6">
              {[
                ...
                { label: 'Active', value: leadConversion.active, color: 'text-[#B8895A]' },
              ].map(s => (
                <div key={s.label} className="p-3 rounded-xl bg-[#FAFAFA] border border-[#E8E2DA] text-center">
                  <p className="text-xs text-[#8C857C]">{s.label}</p>
            ...
            <div className="space-y-2">
              {[
                ...
                { label: 'Active', pct: leadConversion.total ? (leadConversion.active / leadConversion.total) * 100 : 0, color: '#B8895A' },
                { label: 'Lost', pct: leadConversion.total ? (leadConversion.lost / leadConversion.total) * 100 : 0, color: '#EF4444' },
              ].map(s => (
                <div key={s.label} className="flex items-center gap-3">
                  <span className="text-xs text-[#8C857C] w-12 text-right">{s.label}</span>
                  <div className="flex-1 h-5 bg-[#F5F0EB] rounded-lg overflow-hidden">
            ...
            <div className="mt-4 p-3 rounded-xl bg-[#B8895A]/5 border border-[#B8895A]/20 text-center">
              <p className="text-xs text-[#8C857C]">Avg Deal Size</p>
              <p className="text-lg font-bold text-[#B8895A]">{formatCurrency(avgDealSize)}</p>
            </div>
          </div>
        </motion.div>
      </div>
