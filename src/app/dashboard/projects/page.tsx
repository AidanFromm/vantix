...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; border: string }> = {
  lead: { label: 'Lead', color: 'text-[#8B6B56]', bg: 'bg-[#8B6B56]/10', border: 'border-[#8B6B56]/30' },
...
function ProjectModal({ project, clients, onClose, onSave }: { project?: Project | null; clients: Client[]; onClose: () => void; onSave: (data: Partial<Project>) => void }) {
  ...
  const inputCls = 'w-full bg-[#F0DFD1] border border-[#E0CCBA] rounded-xl px-4 py-3 text-sm text-[#4A2112] focus:outline-none focus:border-[#6B3A1F]/50 transition-colors';
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="relative w-full max-w-lg bg-white border border-[#E0CCBA] rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-[#E0CCBA] flex items-center justify-between">
          <h2 className="text-lg font-semibold text-[#4A2112]">{project ? 'Edit Project' : 'New Project'}</h2>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-[#E8D5C4] text-[#8B6B56]"><X size={18} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div><label className="text-xs text-[#8B6B56] mb-1.5 block">Project Name *</label><input type="text" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required className={inputCls} /></div>
          <div><label className="text-xs text-[#8B6B56] mb-1.5 block">Client</label>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className="text-xs text-[#8B6B56] mb-1.5 block">Budget</label><input type="number" value={form.budget} onChange={e => setForm(p => ({ ...p, budget: parseFloat(e.target.value) || 0 }))} className={inputCls} /></div>
            <div><label className="text-xs text-[#8B6B56] mb-1.5 block">Spent</label><input type="number" value={form.spent} onChange={e => setForm(p => ({ ...p, spent: parseFloat(e.target.value) || 0 }))} className={inputCls} /></div>
            <div><label className="text-xs text-[#8B6B56] mb-1.5 block">Progress %</label><input type="number" min={0} max={100} value={form.progress} onChange={e => setForm(p => ({ ...p, progress: parseInt(e.target.value) || 0 }))} className={inputCls} /></div>
          </div>
          <div><label className="text-xs text-[#8B6B56] mb-1.5 block">Deadline</label><input type="date" value={form.deadline} onChange={e => setForm(p => ({ ...p, deadline: e.target.value }))} className={inputCls} /></div>
          <div><label className="text-xs text-[#8B6B56] mb-1.5 block">Description</label><textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={3} className={inputCls + ' resize-none'} /></div>
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving || !form.name.trim()} className="flex-1 px-4 py-3 rounded-xl bg-[#6B3A1F] text-white font-medium text-sm disabled:opacity-50">{project ? 'Update' : 'Create'}</button>
          </div>
        </form>
      </motion.div>
    </motion.div>
...
export default function ProjectsPage() {
  ...
  if (loading) return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{[...Array(4)].map((_, i) => <div key={i} className="h-24 bg-[#E0CCBA]/50 rounded-2xl" />)}</div>
    </div>
  );
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-2xl sm:text-3xl font-bold text-[#4A2112]">Projects</h1><p className="text-sm text-[#8B6B56] mt-1">Track and manage all your projects</p></div>
        <button onClick={() => { setEditingProject(null); setShowModal(true); }} className="flex items-center gap-2 bg-[#6B3A1F] hover:bg-[#A67A4B] text-white px-5 py-2.5 rounded-xl font-medium transition-all shadow-lg shadow-[#6B3A1F]/20 text-sm"><Plus size={18} /> New Project</button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Total Budget', value: formatCurrency(stats.totalBudget), icon: DollarSign, color: 'text-[#6B3A1F]', bg: 'bg-[#6B3A1F]/10', border: 'border-[#6B3A1F]/20' },
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="relative flex-1 sm:max-w-sm">
          ...
          <input type="text" placeholder="Search projects..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-white border border-[#E0CCBA] rounded-xl text-sm text-[#4A2112] focus:outline-none focus:border-[#6B3A1F]/50" />
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-white border border-[#E0CCBA] rounded-xl px-3 py-2 text-sm text-[#4A2112] focus:outline-none focus:border-[#6B3A1F]/50">
      ...
      {filtered.length === 0 && !search && statusFilter === 'all' ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-20 h-20 rounded-2xl bg-[#6B3A1F]/10 flex items-center justify-center mb-6"><Briefcase size={40} className="text-[#6B3A1F]" /></div>
          <h3 className="text-xl font-semibold text-[#4A2112] mb-2">No projects yet</h3>
          <p className="text-[#8B6B56] max-w-md mb-6">Create your first project to start tracking work.</p>
          <button onClick={() => { setEditingProject(null); setShowModal(true); }} className="flex items-center gap-2 px-5 py-3 rounded-xl bg-[#6B3A1F]/10 text-[#6B3A1F] border border-[#6B3A1F]/30 font-medium"><Plus size={18} /> Create Your First Project</button>
        </div>
      ) : (
          ...
          {filtered.map((project, i) => {
            ...
            return (
              <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                className="group relative overflow-hidden rounded-2xl bg-white border border-[#E0CCBA] p-5 hover:border-[#6B3A1F]/30 hover:shadow-lg transition-all duration-300 shadow-[4px_4px_12px_#d1cdc7,-4px_-4px_12px_#ffffff]">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-semibold text-[#4A2112] truncate group-hover:text-[#6B3A1F] transition-colors">{project.name}</h3>
                ...
                {/* Progress Bar */}
                <div className="mb-3">
                  <div className="flex justify-between text-xs text-[#8B6B56] mb-1"><span>Progress</span><span className="text-[#4A2112] font-medium">{progress}%</span></div>
                  <div className="h-2.5 bg-[#E8D5C4] rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-700 ${progress >= 80 ? 'bg-emerald-500' : progress >= 50 ? 'bg-blue-500' : 'bg-[#6B3A1F]'}`} style={{ width: `${Math.min(progress, 100)}%` }} />
                  </div>
                </div>

                {/* Budget vs Actual */}
                {project.budget ? (
                  <div className="mb-3 p-2.5 rounded-lg bg-[#F0DFD1] border border-[#E0CCBA]">
                    <div className="flex items-center justify-between text-xs mb-1.5">
                      <span className="text-[#8B6B56]">Budget</span>
                      <span className={`font-semibold ${overBudget ? 'text-red-600' : 'text-[#4A2112]'}`}>{formatCurrency(project.spent)} / {formatCurrency(project.budget)}</span>
                    </div>
                    <div className="h-1.5 bg-[#E0CCBA] rounded-full overflow-hidden">
                ...
                {milestones.length > 0 && (
                  <div className="mb-3 flex items-center gap-1.5">
                    <Flag size={12} className="text-[#8B6B56]" />
                    <div className="flex gap-1">
                      {milestones.slice(0, 5).map((m, mi) => (
                        <div key={mi} className={`w-3 h-3 rounded-full border ${m.done ? 'bg-emerald-500 border-emerald-400' : 'bg-white border-[#E0CCBA]'}`} title={m.name} />
                      ))}
                    </div>
                    <span className="text-[10px] text-[#8B6B56]">{milestones.filter(m => m.done).length}/{milestones.length}</span>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between text-xs mb-3">
                  {project.client ? (
                    <Link href="/dashboard/clients" className="flex items-center gap-1 text-[#6B3A1F] hover:underline"><Users size={12} />{project.client.name}</Link>
                  ) : <span className="text-[#8B6B56]">No client</span>}
                  {days !== null && <div className={`flex items-center gap-1 ${days < 0 ? 'text-red-500' : days <= 7 ? 'text-amber-500' : 'text-[#8B6B56]'}`}><Clock size={12} />{days < 0 ? `${Math.abs(days)}d over` : `${days}d left`}</div>}
                </div>

                <div className="flex items-center gap-2 pt-3 border-t border-[#E0CCBA]">
                  <button onClick={() => { setEditingProject(project); setShowModal(true); }} className="flex-1 text-center px-3 py-2 rounded-lg bg-[#E8D5C4] text-[#8B6B56] hover:text-[#6B3A1F] hover:bg-[#6B3A1F]/10 text-xs font-medium transition-colors">Edit</button>
                  <button onClick={() => handleDelete(project.id)} disabled={deleting === project.id} className="px-3 py-2 rounded-lg bg-[#E8D5C4] text-[#8B6B56] hover:text-red-500 hover:bg-red-50 text-xs transition-colors">
      ...
      {filtered.length === 0 && (search || statusFilter !== 'all') && (
        <div className="text-center py-12"><p className="text-[#8B6B56]">No projects match your filters</p></div>
      )}
