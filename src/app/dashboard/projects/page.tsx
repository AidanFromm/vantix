...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; border: string }> = {
  lead: { label: 'Lead', color: 'text-[#A89F94]', bg: 'bg-[#A89F94]/10', border: 'border-[#A89F94]/30' },
...
function ProjectModal({ project, clients, onClose, onSave }: { project?: Project | null; clients: Client[]; onClose: () => void; onSave: (data: Partial<Project>) => void }) {
  ...
  const inputCls = 'w-full bg-[#F5EFE7] border border-[#D8CFC4] rounded-xl px-4 py-3 text-sm text-[#1E1E1E] focus:outline-none focus:border-[#B07A45]/50 transition-colors';
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="relative w-full max-w-lg bg-white border border-[#D8CFC4] rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div><label className="text-xs text-[#A89F94] mb-1.5 block">Project Name *</label><input type="text" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required className={inputCls} /></div>
          <div><label className="text-xs text-[#A89F94] mb-1.5 block">Client</label>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className="text-xs text-[#A89F94] mb-1.5 block">Budget</label><input type="number" value={form.budget} onChange={e => setForm(p => ({ ...p, budget: parseFloat(e.target.value) || 0 }))} className={inputCls} /></div>
            <div><label className="text-xs text-[#A89F94] mb-1.5 block">Spent</label><input type="number" value={form.spent} onChange={e => setForm(p => ({ ...p, spent: parseFloat(e.target.value) || 0 }))} className={inputCls} /></div>
            <div><label className="text-xs text-[#A89F94] mb-1.5 block">Progress %</label><input type="number" min={0} max={100} value={form.progress} onChange={e => setForm(p => ({ ...p, progress: parseInt(e.target.value) || 0 }))} className={inputCls} /></div>
          </div>
          <div><label className="text-xs text-[#A89F94] mb-1.5 block">Deadline</label><input type="date" value={form.deadline} onChange={e => setForm(p => ({ ...p, deadline: e.target.value }))} className={inputCls} /></div>
          <div><label className="text-xs text-[#A89F94] mb-1.5 block">Description</label><textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={3} className={inputCls + ' resize-none'} /></div>
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving || !form.name.trim()} className="flex-1 px-4 py-3 rounded-xl bg-[#B07A45] text-white font-medium text-sm disabled:opacity-50">{project ? 'Update' : 'Create'}</button>
          </div>
        </form>
      </motion.div>
    </motion.div>
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-2xl sm:text-3xl font-bold text-[#1E1E1E]">Projects</h1><p className="text-sm text-[#A89F94] mt-1">Track and manage all your projects</p></div>
        <button onClick={() => { setEditingProject(null); setShowModal(true); }} className="flex items-center gap-2 bg-[#B07A45] hover:bg-[#A67A4B] text-white px-5 py-2.5 rounded-xl font-medium transition-all shadow-lg shadow-[#B07A45]/20 text-sm"><Plus size={18} /> New Project</button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Total Budget', value: formatCurrency(stats.totalBudget), icon: DollarSign, color: 'text-[#B07A45]', bg: 'bg-[#B07A45]/10', border: 'border-[#B07A45]/20' },
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="relative flex-1 sm:max-w-sm">
          ...
          <input type="text" placeholder="Search projects..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-white border border-[#D8CFC4] rounded-xl text-sm text-[#1E1E1E] focus:outline-none focus:border-[#B07A45]/50" />
        </div>
      ...
      {filtered.length === 0 && !search && statusFilter === 'all' ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-20 h-20 rounded-2xl bg-[#B07A45]/10 flex items-center justify-center mb-6"><Briefcase size={40} className="text-[#B07A45]" /></div>
          <h3 className="text-xl font-semibold text-[#1E1E1E] mb-2">No projects yet</h3>
          <p className="text-[#A89F94] max-w-md mb-6">Create your first project to start tracking work.</p>
          <button onClick={() => { setEditingProject(null); setShowModal(true); }} className="flex items-center gap-2 px-5 py-3 rounded-xl bg-[#B07A45]/10 text-[#B07A45] border border-[#B07A45]/30 font-medium"><Plus size={18} /> Create Your First Project</button>
        </div>
      ) : (
          ...
          {filtered.map((project, i) => {
            ...
            return (
              <motion.div key={project.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                className="group relative overflow-hidden rounded-2xl bg-white border border-[#D8CFC4] p-5 hover:border-[#B07A45]/30 hover:shadow-lg transition-all duration-300 shadow-sm">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-semibold text-[#1E1E1E] truncate group-hover:text-[#B07A45] transition-colors">{project.name}</h3>
                ...
                {/* Progress Bar */}
                <div className="mb-3">
                  <div className="flex justify-between text-xs text-[#A89F94] mb-1"><span>Progress</span><span className="text-[#1E1E1E] font-medium">{progress}%</span></div>
                  <div className="h-2.5 bg-[#EFE6DA] rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-700 ${progress >= 80 ? 'bg-emerald-500' : progress >= 50 ? 'bg-blue-500' : 'bg-[#B07A45]'}`} style={{ width: `${Math.min(progress, 100)}%` }} />
                  </div>
                </div>
                ...
                {project.budget ? (
                  <div className="mb-3 p-2.5 rounded-lg bg-[#F5EFE7] border border-[#D8CFC4]">
                    <div className="flex items-center justify-between text-xs mb-1.5">
                      <span className="text-[#A89F94]">Budget</span>
                ...
                {milestones.length > 0 && (
                  <div className="mb-3 flex items-center gap-1.5">
                    <Flag size={12} className="text-[#A89F94]" />
                    ...
                    <span className="text-[10px] text-[#A89F94]">{milestones.filter(m => m.done).length}/{milestones.length}</span>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between text-xs mb-3">
                  {project.client ? (
                    <Link href="/dashboard/clients" className="flex items-center gap-1 text-[#B07A45] hover:underline"><Users size={12} />{project.client.name}</Link>
                  ) : <span className="text-[#A89F94]">No client</span>}
                  {days !== null && <div className={`flex items-center gap-1 ${days < 0 ? 'text-red-500' : days <= 7 ? 'text-amber-500' : 'text-[#A89F94]'}`}><Clock size={12} />{days < 0 ? `${Math.abs(days)}d over` : `${days}d left`}</div>}
                </div>

                <div className="flex items-center gap-2 pt-3 border-t border-[#D8CFC4]">
                  <button onClick={() => { setEditingProject(project); setShowModal(true); }} className="flex-1 text-center px-3 py-2 rounded-lg bg-[#EFE6DA] text-[#A89F94] hover:text-[#B07A45] hover:bg-[#B07A45]/10 text-xs font-medium transition-colors">Edit</button>
                  <button onClick={() => handleDelete(project.id)} disabled={deleting === project.id} className="px-3 py-2 rounded-lg bg-[#EFE6DA] text-[#A89F94] hover:text-red-500 hover:bg-red-50 text-xs transition-colors">
      ...
      {filtered.length === 0 && (search || statusFilter !== 'all') && (
        <div className="text-center py-12"><p className="text-[#A89F94]">No projects match your filters</p></div>
      )}
