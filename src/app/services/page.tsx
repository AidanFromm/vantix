    ...
    <div className="min-h-screen bg-[#F0DFD1] text-[#2C1810] scroll-smooth">
      ...
      <section className="max-w-6xl mx-auto px-6 pt-20 pb-16 text-center">
        <motion.p
          ...
          className="text-[#6B3A1F] text-sm font-semibold uppercase tracking-widest mb-4"
        >
          What We Deploy
        </motion.p>
        <motion.h1
        ...
        <motion.p
          ...
          className="text-lg text-[#8B6B56] max-w-2xl mx-auto"
        >
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {services.map((s, i) => (
            <motion.div
              ...
              className="rounded-2xl p-8 bg-white border border-[#E0CCBA] shadow-[8px_8px_20px_#d1cdc7,-8px_-8px_20px_#ffffff] hover:shadow-[12px_12px_28px_#c8c4be,-12px_-12px_28px_#ffffff] hover:border-[#6B3A1F]/20 transition-all duration-300"
            >
              <div className="w-14 h-14 rounded-xl bg-[#F0DFD1] shadow-[inset_4px_4px_8px_#d1cdc7,inset_-4px_-4px_8px_#ffffff] flex items-center justify-center mb-6">
                <s.icon className="w-7 h-7 text-[#6B3A1F]" />
              </div>
              <h3 className="text-2xl font-bold mb-2">{s.title}</h3>
              <p className="text-[#6B3A1F] text-sm font-semibold mb-4">{s.price}</p>
              <p className="text-[#8B6B56] mb-6 leading-relaxed">{s.description}</p>
              <ul className="space-y-2.5 mb-6">
                {s.features.map((f) => (
                  <li key={f} className="flex items-start gap-2.5 text-sm text-[#5C5650]">
                    <CheckCircle2 className="w-4 h-4 text-[#6B3A1F] mt-0.5 shrink-0" />
                    {f}
                  </li>
                ))}
              </ul>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24 text-center">
        ...
        >
          <Calendar className="w-10 h-10 text-[#6B3A1F] mx-auto mb-4" />
