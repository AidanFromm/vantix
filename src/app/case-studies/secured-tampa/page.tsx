    ...
    <div className="min-h-screen bg-[#F5EFE7] text-[#1E1E1E] scroll-smooth">
      ...
      <section className="max-w-4xl mx-auto px-6 pt-20 pb-12">
        <motion.div initial="hidden" animate="visible" variants={fadeUp}>
          <p className="text-xs font-semibold text-[#B07A45] uppercase tracking-wider mb-3">Client Case Study</p>
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4">
            Shopify Shut Them Down.<br />We Built Something Better in 3 Weeks.
          </h1>
          <p className="text-lg text-[#A89F94] max-w-2xl">
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((s) => (
            <div key={s.label} className="rounded-2xl p-6 text-center bg-white border border-[#D8CFC4] shadow-[6px_6px_16px_#d1cdc7,-6px_-6px_16px_#ffffff]">
              ...
              <p className="text-3xl font-bold text-[#B07A45]">{s.value}</p>
              <p className="text-sm text-[#A89F94] mt-1">{s.label}</p>
            </div>
          ))}
        </motion.div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
          ...
          <p className="text-[#B07A45] text-sm font-medium mb-6">Built from scratch. Deployed in production. Running 24/7.</p>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={item.text} className="flex items-start gap-3 p-3 rounded-xl hover:bg-[#F5EFE7] transition-colors">
                <item.icon className="w-5 h-5 text-[#B07A45] mt-0.5 shrink-0" />
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {techStack.map((t) => (
              <span key={t} className="px-5 py-2.5 rounded-full text-sm font-medium bg-white border border-[#D8CFC4] shadow-sm hover:border-[#B07A45]/20 transition-colors">
                {t}
              </span>
            ))}
          </div>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
          className="rounded-2xl p-8 md:p-10 bg-white border border-[#B07A45]/20 shadow-[8px_8px_20px_#d1cdc7,-8px_-8px_20px_#ffffff]">
          ...
          <ul className="space-y-3 mb-8">
            {[
              ...
              <li key={r} className="flex items-start gap-3 text-[#5C5650]">
                <CheckCircle2 className="w-5 h-5 text-[#B07A45] mt-0.5 shrink-0" />
                <span>{r}</span>
              </li>
            ))}
          </ul>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
          ...
          <MessageSquare className="w-8 h-8 text-[#B07A45]/30 mx-auto mb-4" />
