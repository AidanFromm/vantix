    ...
    <div className="min-h-screen bg-[#F5EFE7] text-[#1E1E1E] scroll-smooth">
      <nav className="sticky top-0 z-50 bg-[#F5EFE7]/90 backdrop-blur-md border-b border-[#D8CFC4]">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/case-studies" className="flex items-center gap-2 text-lg font-bold tracking-tight hover:text-[#9B6C3C] transition-colors">
      ...
      <section className="max-w-4xl mx-auto px-6 pt-20 pb-12">
        <motion.div initial="hidden" animate="visible" variants={fadeUp}>
          <p className="text-xs font-semibold text-[#9B6C3C] uppercase tracking-wider mb-3">Client Case Study</p>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((s) => (
            <div key={s.label} className="rounded-2xl p-6 text-center bg-white border border-[#D8CFC4] shadow-[6px_6px_16px_#d1cdc7,-6px_-6px_16px_#ffffff]">
              <s.icon className="w-5 h-5 text-[#9B6C3C] mx-auto mb-2" />
              <p className="text-3xl font-bold text-[#9B6C3C]">{s.value}</p>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
          className="rounded-2xl p-8 md:p-10 bg-white border border-[#D8CFC4] shadow-[8px_8px_20px_#d1cdc7,-8px_-8px_20px_#ffffff]">
          ...
          <div className="space-y-4 text-[#5C5650] leading-relaxed">
            ...
            <p><strong className="text-[#1E1E1E]">The clock was ticking.</strong> Every day without a platform meant lost sales, frustrated customers, and competitors gaining ground. He needed a complete solution — fast.</p>
          </div>
        </motion.div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
          ...
          <p className="text-[#9B6C3C] text-sm font-medium mb-6">Built from scratch. Deployed in production. Running 24/7.</p>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={item.text} className="flex items-start gap-3 p-3 rounded-xl hover:bg-[#F5EFE7] transition-colors">
                <item.icon className="w-5 h-5 text-[#9B6C3C] mt-0.5 shrink-0" />
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {techStack.map((t) => (
              <span key={t} className="px-5 py-2.5 rounded-full text-sm font-medium bg-white border border-[#D8CFC4] shadow-sm hover:border-[#9B6C3C]/20 transition-colors">
                {t}
              </span>
            ))}
          </div>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
          className="rounded-2xl p-8 md:p-10 bg-white border border-[#9B6C3C]/20 shadow-[8px_8px_20px_#d1cdc7,-8px_-8px_20px_#ffffff]">
          ...
          <ul className="space-y-3 mb-8">
            {[
              ...
              <li key={r} className="flex items-start gap-3 text-[#5C5650]">
                <CheckCircle2 className="w-5 h-5 text-[#9B6C3C] mt-0.5 shrink-0" />
                <span>{r}</span>
              </li>
            ))}
          </ul>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
          className="rounded-2xl p-8 md:p-10 bg-white border border-[#D8CFC4] shadow-[8px_8px_20px_#d1cdc7,-8px_-8px_20px_#ffffff] text-center">
          <MessageSquare className="w-8 h-8 text-[#9B6C3C]/30 mx-auto mb-4" />
          <blockquote className="text-xl md:text-2xl text-[#1E1E1E] font-medium leading-relaxed mb-6 max-w-2xl mx-auto">
          ...
          <p className="text-[#1E1E1E] font-semibold">Dave</p>
