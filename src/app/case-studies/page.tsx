          ...
          {caseStudies.map((cs, i) => (
            ...
            >
              <Link href={`/case-studies/${cs.slug}`} className="block group">
                <div className="rounded-2xl p-8 bg-[#F0DFD1] border border-[#E0CCBA] shadow-[8px_8px_20px_#d1cdc7,-8px_-8px_20px_#ffffff] hover:shadow-[12px_12px_28px_#c8c4be,-12px_-12px_28px_#ffffff] transition-shadow duration-300">
                  <div className="w-14 h-14 rounded-xl bg-[#F0DFD1] shadow-[inset_4px_4px_8px_#d1cdc7,inset_-4px_-4px_8px_#ffffff] flex items-center justify-center mb-6">
                    <cs.icon className="w-7 h-7 text-[#6B3A1F]" />
                  </div>
                  <p className="text-xs font-semibold text-[#6B3A1F] uppercase tracking-wider mb-2">{cs.industry}</p>
                  <h3 className="text-2xl font-bold mb-2">{cs.client}</h3>
                  <p className="text-[#8B6B56] text-sm mb-4 leading-relaxed">{cs.description}</p>
                  <div className="flex items-center gap-2 mb-4 p-3 rounded-xl bg-[#F0DFD1] shadow-[inset_3px_3px_6px_#d1cdc7,inset_-3px_-3px_6px_#ffffff]">
                    <TrendingUp className="w-4 h-4 text-[#6B3A1F] shrink-0" />
                    <span className="text-sm font-semibold">{cs.metric}</span>
                  </div>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {cs.tags.map((t) => (
                      <span key={t} className="text-xs px-3 py-1 rounded-full border border-[#E0CCBA] text-[#8B6B56]">{t}</span>
                    ))}
                  </div>
                  <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#6B3A1F] group-hover:text-[#9A7048] transition-colors">
